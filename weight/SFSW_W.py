...
if __name__=='__main__':
    ...
    ts_volume = np.array(TS_x['Open'])
    ts = ts.tolist()
    ts_volume.tolist()
    ts_x = np.array(TS_x['Date'])
    TS_x = np.array(TS_x['Date']) 
    TS_1 = np.array(TS['Close'])
    TS_volume = np.array(TS['Open'])    
    ...
    # =============================================================================
    #     for i in range(len(segList_SFSW)):
    #         ans_seg_point_date.append(ts_x[ts.index(segList_SFSW[i])])
    # =============================================================================
        segList_SFSW = segList_SFSW_1
    # =============================================================================
    #     plt.figure(figsize=(12,6))
    #     plt.plot(ts_x,ts,'r--',label='raw_data') 
    #     plt.scatter(ans_seg_point_date,segList_SFSW,color='blue',label='seg_point')
    #     plt.plot(ans_seg_point_date,segList_SFSW,color='green',label='fitting_line')
    #     plt.legend()
    #     plt.xticks(ts_x, ts_x, rotation=90, fontsize=10)
    #     plt.show() 
    # =============================================================================
        for i in range(len(ts)):
            ts[i] = ts[i] * 0.5 + ts[i] * 0.5
    ...
    np.savetxt('E:/论文/论文初写/code/output/CSIW/CSIW_SFSW.txt',list_ans,fmt='%.4f')  